...
const getPath = (htPath, program) =>
  path.join(program.directory, htPath, '.well-known/security.txt');
...
exports.onPostBuild = async ({ store }, pluginOptions) => {
  ...
  const htContent = getContent(
    path.join(__dirname, 'utils/files/security.txt'),
    pluginOptions
  );
  ...
  try {
    ...
    console.error(
      'onPostBuild error gatsby-plugin-security-txt',
      JSON.stringify(e)
    );
    return;
  }
